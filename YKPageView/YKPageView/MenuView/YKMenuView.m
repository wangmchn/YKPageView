...
@property (nonatomic, weak) YKProgressView *progressView;
@property (nonatomic, weak) YKMenuItem *selItem;
@property (nonatomic, strong) UIColor *bgColor;
@property (nonatomic, strong) NSMutableArray *frames;
@end
// 下划线的高度
static CGFloat const YKProgressHeight = 2.0;
...
- (void)slideMenuAtProgress:(CGFloat)progress {
    ...
    YKMenuItem *currentItem = (YKMenuItem *)[self viewWithTag:tag];
    YKMenuItem *nextItem = (YKMenuItem *)[self viewWithTag:tag+1];
    if (rate == 0.0) {
        rate = 1.0;
        self.selItem.rate = 0;
...
- (void)selectItemAtIndex:(NSInteger)index {
    ...
    YKMenuItem *item = (YKMenuItem *)[self viewWithTag:tag];
    [self.selItem deselectedItemWithoutAnimation];
    self.selItem = item;
    [self.selItem selectedItemWithoutAnimation];
...
- (void)addItems {
    ...
    for (int i = 0; i < self.items.count; i++) {
        ...
        YKMenuItem *item = [[YKMenuItem alloc] initWithFrame:frame];
        item.tag = (i+kTagGap);
        item.delegate = self;
...
- (void)calculateItemFrames {
    CGFloat contentWidth = YKMenuMargin;
    for (int i = 0; i < self.items.count; i++) {
...
- (void)addProgressView {
    YKProgressView *pView = [[YKProgressView alloc] initWithFrame:CGRectMake(0, self.frame.size.height - YKProgressHeight, self.scrollView.contentSize.width, YKProgressHeight)];
...
- (void)addFooldViewHollow:(BOOL)isHollow {
    YKFooldView *fooldView = [[YKFooldView alloc] initWithFrame:CGRectMake(0, 0, self.scrollView.contentSize.width, self.frame.size.height)];
